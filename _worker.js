    ...
    async fetch(request, env) {
        try {
            ...
            if (fileName === "" || fileName === "/") {
                const tvContent = await env.KV.get("tv.json", { cacheTtl: 60 });
                if (tvContent === null) {
                    return createResponse('tv.json 文件未找到', 404);
                }
                return createResponse(tvContent, 200, {
...
async function handleFileOperation(KV, fileName, url, token) {
 ...
 // 将文件名统一转换为小写
    const kvKey = fileName.toLowerCase();
    // 写入操作需验证 token
    if (text || b64) {
      if (token !== mytoken) {
        return createResponse('写入操作需要有效 token', 403);
        }
        const content = text || base64Decode(replaceSpacesWithPlus(b64));
        await KV.put(kvKey, content);
        const verifiedContent = await KV.get(kvKey, { cacheTtl: 60 });
        if (verifiedContent !== content) {
            throw new Error('内容写入后校验失败');
        }
        return createResponse(verifiedContent);
    }

// 否则为只读模式
    const value = await KV.get(kvKey, { cacheTtl: 60 });
    if (value === null) {
        return createResponse('文件未找到', 404);
    }
    return createResponse(value);
}
            switch (fileName) {
                // 新增 case，处理直接访问 mytoken 的情况
                case mytoken:
                    return createResponse(configHTML(url.hostname, mytoken), 200, { 'Content-Type': 'text/html; charset=UTF-8' });
                case "config":
                    return createResponse(configHTML(url.hostname, mytoken), 200, { 'Content-Type': 'text/html; charset=UTF-8' });
                case "config/update.bat":
                    return createResponse(generateBatScript(url.hostname, mytoken), 200, {
                        "Content-Disposition": 'attachment; filename=update.bat',
                        "Content-Type": "text/plain; charset=utf-8"
                    });
                case "config/update.sh":
                    return createResponse(generateShScript(url.hostname, mytoken), 200, {
